...
import ExcelLoom.excel.annotation.Picture;
import ExcelLoom.excel.exception.PictureException;

public class SheetPictureHandler extends Anchor {
    public SheetPictureHandler(Sheet sheet,Class<?> clazz) {
        super(sheet);
        if (!clazz.isAnnotationPresent(ExcelLoom.excel.annotation.Sheet.class) || Objects.isNull(clazz.getAnnotation(ExcelLoom.excel.annotation.Sheet.class).picture()) || StringUtils.isBlank(clazz.getAnnotation(ExcelLoom.excel.annotation.Sheet.class).picture().path()))
            return;
        Picture picture = clazz.getAnnotation(ExcelLoom.excel.annotation.Sheet.class).picture();
