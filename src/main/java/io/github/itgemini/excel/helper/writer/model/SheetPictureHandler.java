...
import io.github.itgemini.excel.annotation.Picture;
import io.github.itgemini.excel.exception.PictureException;

public class SheetPictureHandler extends Anchor {
    public SheetPictureHandler(Sheet sheet,Class<?> clazz) {
        super(sheet);
        if (!clazz.isAnnotationPresent(io.github.itgemini.excel.annotation.Sheet.class) || Objects.isNull(clazz.getAnnotation(io.github.itgemini.excel.annotation.Sheet.class).picture()) || StringUtils.isBlank(clazz.getAnnotation(io.github.itgemini.excel.annotation.Sheet.class).picture().path()))
            return;
        Picture picture = clazz.getAnnotation(io.github.itgemini.excel.annotation.Sheet.class).picture();
